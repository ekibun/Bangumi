...
public class FastScrollPopup {
    ...
    FastScrollPopup(Resources resources, FastScrollRecyclerView recyclerView) {
        ...
        setTextSize(ResourceUtil.INSTANCE.toScreenPixels(44));
        setBackgroundSize(ResourceUtil.INSTANCE.toPixels(88));
    }
    ...
    public Rect updateFastScrollerBounds(FastScrollRecyclerView recyclerView, int thumbOffsetY) {
        ...
        if (isVisible()) {
            ...
            if (mPosition == FastScroller.FastScrollerPopupPosition.CENTER) {
                ...
                mBgBounds.top = Math.max(edgePadding + recyclerView.getScrollTopMargin(), Math.min(mBgBounds.top, recyclerView.getHeight() - edgePadding - bgHeight));
            }
